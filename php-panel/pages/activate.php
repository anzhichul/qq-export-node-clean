...
$exp = membership_expires_at();
$balance = 0;
$stmt = db()->prepare('SELECT balance_points FROM users WHERE id=?');
$stmt->execute([intval($_SESSION['user_id'])]);
$balance = intval($stmt->fetchColumn() ?: 0);
$active = $exp > now();
?>
<h2 class="page-title">激活中心</h2>
...
<div class="stats">
  <div class="stat">
    <b style="color:<?=$active?'#1d7a42':'#c73d3d'?>"><?=$exp>0?($active?'会员中':'已过期'):'未激活'?></b>
    <small>会员状态</small>
  </div>
  <div class="stat">
    <b><?=$exp>0?date('Y-m-d H:i', $exp):'-'?></b>
    <small>到期时间</small>
  </div>
  <div class="stat"><b><?=$balance?></b><small>当前点数</small></div>
</div>

<?php if (!$active): ?>
<div class="card" style="border:1px solid #d8b23a;background:#fffbea">
  <h3 style="color:#8a6d1a">账号未激活</h3>
  <p style="color:#6d7c8d;font-size:13px">你的账号尚未激活或会员已到期，当前无法使用平台功能。<br>请输入卡密激活：</p>
  ...
  <?php if ($purchaseUrl !== ''): ?>
  <div style="margin-top:14px;padding-top:14px;border-top:1px dashed #e0c878">
    <p style="color:#8a6d1a;font-size:13px;margin-bottom:8px">还没有卡密？</p>
...
<div class="card">
  <h3>卡密充值 / 续费</h3>
  <p style="color:#6d7c8d;font-size:13px">时长卡用于续费会员，点数卡用于增加发信点数。</p>
  <form method="post" style="display:flex;gap:8px;align-items:center;flex-wrap:wrap;margin-top:10px">
    <input type="hidden" name="action" value="redeem">
    <input name="code" placeholder="输入卡密，如 LK-XXXXX-XXXXX-XXXXX-XXXXX" style="flex:1;min-width:220px;text-transform:uppercase" required>
